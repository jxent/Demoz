...
public class HVScrollView extends FrameLayout {
    ...
    public HVScrollView(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.HvScrollView);
        mViewIndex = a.getInteger(R.styleable.HvScrollView_viewIndex, -1);
		a.recycle();
        initScrollView();   
    }   
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_DOWN: {
            ...
            mLastMotionX = x;   
            mActivePointerId = ev.getPointerId(0);   
            setViewTouchDownEventListenerForIndex();
            break;   
        }   
        case MotionEvent.ACTION_MOVE:
            if (mIsBeingDragged) {   
    ...
    private void setViewTouchDownEventListenerForIndex(){
    	// 0 --- main   1---left   2--top  3---below
    	mTouchEventListener.onTouchUpMain();
    	mTouchEventListener.onTouchUpLeft();
    	mTouchEventListener.onTouchUpTop();
    	mTouchEventListener.onTouchUpBelow();
    	if(mViewIndex == 0){
    		mTouchEventListener.onTouchDownMain();
    	}else if(mViewIndex == 1){
    		mTouchEventListener.onTouchDownLeft();
    	}else if(mViewIndex == 2){
    		mTouchEventListener.onTouchDownTop();
    	}else if(mViewIndex == 3){
    		mTouchEventListener.onTouchDownBelow();
    	}
    }
    ...
    public void setTouchEventListener(TouchEventListener taEventListener){
    	mTouchEventListener = taEventListener;
    }
